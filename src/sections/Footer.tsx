    ...
    return (
        <footer className={`relative -z-10 overflow-x-clip`}>
            ...
            <div className={`container`}>
                <div className={`border-t border-text/40 py-4 text-sm  flex flex-col md:flex-row md:justify-between items-center gap-8`}>
                    <div className={`text-text/70`}>&copy; 2025. All right reserved.</div>
                    <nav className={`flex flex-col md:flex-row items-center gap-8`}>
                        {footerLinks.map(link => (
                            <a key={link.title} className={`inline-flex items-center gap-1.5`} href={link.url}>
